...
public sealed class ProjectsRepository(ApplicationDbContext context) : IProjectsRepository
{
    ...
    public async Task<ProjectRegistrationApproval> GetApproval(
        ProjectName name,
    ...
    )
    {
        ...
        if (!exists)
            return new ProjectRegistrationApproval(true);
    ...
    public async Task<Result<Project>> GetProject(
        Guid id,
        bool withLock = false,
        CancellationToken ct = default
    )
    {
        Result<ProjectId> idVo = ProjectId.Create(id);
        if (idVo.IsFailure)
            return Error.NotFound("Проект не найден.");

        if (withLock)
            await BlockProjectRow(idVo.OnSuccess, ct);
        ...
        return project is null ? Error.NotFound("Проект не найден.") : project;
    }
    ...
    public async Task<bool> Exists(ProjectTaskAssignment assignment, CancellationToken ct = default)
    {
        ...
        AssignmentExistsResult result = await Context
            .Database.SqlQueryRaw<AssignmentExistsResult>(
                @"
