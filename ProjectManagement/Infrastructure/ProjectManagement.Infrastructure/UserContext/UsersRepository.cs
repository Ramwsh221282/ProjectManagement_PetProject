...
{
    public async Task<Result<User>> GetUser(Guid id, CancellationToken ct = default)
    {
        Result<UserId> userId = UserId.Create(id);
        if (userId.IsFailure)
            return Error.NotFound("Пользователь не найден.");

        User? user = await context.Users.FirstOrDefaultAsync(
            u => u.UserId == userId.OnSuccess,
            cancellationToken: ct
        );

        return user is null ? Error.NotFound("Пользователь не найден.") : user;
    }
    ...
    )
    {
        ...
        RegistrationCheckResult result = await context
            .Database.SqlQueryRaw<RegistrationCheckResult>(
                @"
            SELECT
